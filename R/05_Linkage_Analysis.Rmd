...
As the set of SNPs under analysis had been pruned in order to only include one representative SNP in the case of linkage, any SNPs in linkage to the detected SNPs were explored.
In addition, SNPs in the same locus which were not in linkage were explored.
Correlations were calculated for each timepoint separately as they may not be expected to be the same at each timepoint.

```{r linkageSnpSetup, cache=TRUE}
...
As might be expected, linkage would be expected to decrease over the passage of time through recombination, as seen below, where correlations (as indicated by the blue regression line) are trending towards zero.
...
Only one pair of SNPs appeared to increase their linkage to 100% post-RHDV (`r cors %>% dplyr::filter(corr2012 ==1, corr1996 != 1, chromosome == "GL018883") %>% dplyr::select(starts_with("snp")) %>% unlist() %>% as.character() %>% pander()`).
These are at a distance of `r cors %>% dplyr::filter(corr2012 ==1, corr1996 != 1, chromosome == "GL018883") %>% .[["dist"]]`bp away, with the initial snp (3999128_75), being identifed as intronic for the gene *SERPINA6*.
This putatively linked SNP is also within the coding region for this gene, however this is also intronic.
