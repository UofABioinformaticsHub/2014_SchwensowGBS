...
```{r allLD, results='hide', include=FALSE}
seqSetFilter(gdsFile, sample.id = sampleID$Sample)
allLD <- snps %>% 
  filter(
...
```{r plotDecay, cache=TRUE, fig.cap = "*Linkage decay for distances up to 1Mb. The Hill-Weir decay line is shown in green as is the distance 40kb. The red dashed line indicates the distances where linkage falls to half of the maximal linkage.*"}
allLD %>%
...
sigCors <- cors %>%
  ...
  mutate(
    ...
    p = case_when(
      !is.nan(Z) ~ pnorm(abs(Z), lower.tail = FALSE)*2, 
      is.nan(Z) ~ 1
    ),
    adjP = p.adjust(p, "fdr")
    ) %>%
...
Correlations were converted to Z scores using Fisher's transformation and tests for a significant change were conducted.
P-values were adjusted using the Benjamini-Hochbers False-Discovery Rate, giving `r sum(sigCors$adjP < 0.05)` SNPs with significantly changed correlations.
...
sigCors %>% 
  ...
  pander(
    caption = paste(
      "*SNPs showing a significant shift in linkage to an FDR of 5%.",
      "The position given corresponds to the SNP considered as potentially",
      "impacted by RHDV selection, whilst the the distance refers to the SNP",
      "where linkage has been potentially impacted by RHDV selection.*"
    )
  )
```
...
However, it remains possible that this apparent increase in linkage across a plausible distance may be evidence of selection.

One further SNP pair showed a noticeable increase in linkage (2405359_66/2405410_23), with both of these SNPs being intronic within ENSOCUG00000005835 (*STXBP5L*).
This SNP showed a signifcant increase in the MAF (1996 - 0.021; 2012 - 0.202) and this relatively low frequency does explain some of the apparent decrease in correlation for the SNP within the same locus (2405359_68).

One SNP pair switched from a negative correlation to a positive correlation, which given the strong shift towards the homozygous genotype for the major allele, may again appear consistent with selection.
These SNPs are both within 20kb of the gene *RHOQ*, which along with *STXBP5L* has been connected to insulin secretion & the related response.
No obvious link can be drawn to any insulin-related pathways at this point

### SNPs Showing A Noticeable Decrease in Linkage 

In addition to the above SNPs, four SNPs around 2028473_73 (13:2979383) were detected as having a significant decrease in linkage.
It may be postulated that this is evidence of diversifying selection, and this may be additionally supported by the observation of an increase in the minor allele frquency at this locus in the post-RHDV population.
In addition, the lower 2012 correlations upstream of this SNP, compared to the downstream SNPs may indicate selection acting on a region more distal.
Whilst no genes defined in Ensembl 96 were within 100kb of this SNP, a Genscan predicted transcript (GENSCAN00000009622.1) directly overlapped this gene, and the predicted cDNA shared some similarity with *tenascin R* from *Gorilla gorilla* (11% cover; 92% identity; E = 3e-43).


### SNPs Within the Same Locus

```{r sameLocTable}
dplyr::filter(cors, Locus1 == Locus2) %>% 
  dplyr::select(chromosome, snp1, snp2, pos1, dist,corr1996, corr2012) %>% 
  pander(
    caption = paste(
      "Correlations between SNPs within the same loci as candidate SNPs detected earlier.*"
    )
  )
```
