...
snpSummary %>%
    mutate_at(sampleID$Sample, funs(is.na)) %>%
...
- This has left `r comma(sum(vapply(snpset, length, integer(1))))` SNPs as candidates for downstream analysis
...
```{r}
write_rds(snpset, "snpsetAfterLDPruning.RDS")
```
