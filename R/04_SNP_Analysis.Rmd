...
Of the initial `r comma(length(unique(snpsGR$snpID)))` SNPs, `r comma(length(unique(snp2Gene$snpID)))` were found to be within `r maxKb`KB of a gene.
...
```{r map2Pdf, echo=FALSE, cache=TRUE, dependson=c('mainPlot', 'Map4Inset')}
...
list(
  ...
  Allele = alleleResults %>%
    ...
    dplyr::filter(FDR < alpha) %>%
    .[["variant.id"]],
  Bayescan = dplyr::filter(bayRes, qval < 0.05, variant.id %in% snp2Gene$variant.id)$variant.id
) %>% 
