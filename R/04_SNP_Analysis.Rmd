...
snpIn1996 <- genotypes %>% 
    ...
    group_by(variant.id) %>% 
    summarise(maf = 1 - mean(Genotype) / 2) %>%
    filter(maf > 0)
genotypes %<>% 
...
A total of `r sum(regionResults$p < 0.05, na.rm = TRUE)` SNPs were detected as showing a significant association between genotype and the collection region.
...
Under this additional filtering step, **the original set of `r nrow(regionResults)` SNPs will be reduced to `r comma(sum(regionResults$p >= 0.05, na.rm = TRUE))`** for testing by genotype and allele frequency.
...
hLight <- filter(genotypeResults, adjP < alpha)$variant.id
genotypeResults %>%
...
hLight <- as.character(filter(alleleResults, adjP < 0.05)$variant.id)
alleleResults %>%
  mutate(Chr = as.numeric(chromosome),
