...
```{r plotVenn, echo=FALSE, fig.cap = "*Summary of results from all three methods, using an FDR of 0.05 for each approach. All SNPs detected under Bayescan were identified using Fisher's Exact Test for both allele and genotype analysis*"}
grid.newpage()
list(
...
For the initial GO analysis, any gene within `r dist/1000`kb of a SNP considered as significant were tested for any enrichment of biological characteristics.
...
`r nGenes` genes were identified as being within `r dist/1000`kb of a SNP.
Of the `r length(unique(sigSnpGR$snpID))` candidate SNPs `r nSigGenes` genes were identified as being within 40kb, and these were analysed for enrichment of any GO terms.
...
combinedResults %>%
  ...
  pander(justify = "lrrrrr",
         caption = "Significant SNPs from the meta-analysis using the previous published results from a separate population.")
```
...
allSnpsGR %>% 
  ...
  pander(style = "rmarkdown", split.table = Inf, justify = "lrrlll",
         caption = paste0(
           "Genes within ", dist / 1000, "kb of SNPs identified in the meta-analysis."))
```

```{r, echo=FALSE, fig.cap="Comparison of p-values for SNPs detected in both analyses, with thos considered as significant after meta-analysis shown in red."}
combinedResults %>%
  mutate(Sig = FDR < 0.05) %>%
...
```{r, echo=FALSE, fig.cap = "Allele distributions in the 1996 and 2012 populations for SNPs identified under the meta-analysis."}
combinedResults %>%
  dplyr::filter(FDR < 0.05) %>%
  dplyr::select(snpID) %>%
  left_join(genotypes) %>%
...
combinedGoRes %>%
  ...
  pander(split.tables = Inf, style = "rmarkdown", 
         justify = "lllrrrrrrl",
         caption = paste("Most highly ranked GO terms for combined results.", 
                         "Where multiple terms resulted from the identical combination of genes, only the most highly ranked terms is shown.",
                         "Overall, this corresponds to an FDR of", 
                         percent(max(.$FDR))))
```
