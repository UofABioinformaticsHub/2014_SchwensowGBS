...
- The mean distance between SNPs in the final dataset was `r genotypes %>% distinct(snpID, .keep_all = TRUE) %>% split(f = .$chromosome) %>% lapply(function(x){diff(x$position)}) %>% unlist() %>% mean() %>% divide_by(1000) %>% round(2)`kb
...
```{r central}
central <- list(
  Latitude = c(-31.25, -31.2373),
  Longitude = c(138.7535, 138.7495)
)
sampleID %<>%
    mutate(
    splitPop = case_when(
      ...
      Latitude > max(central$Latitude) ~ "2012 (Outer)",
      Latitude < min(central$Latitude) ~ "2012 (Outer)",
      Longitude > max(central$Longitude) ~ "2012 (Outer)",
      Longitude < min(central$Longitude) ~ "2012 (Outer)",
      Latitude < max(central$Latitude) &
        Latitude > min(central$Latitude) & 
        Longitude < max(central$Longitude) &
        Longitude > min(central$Longitude) ~ "2012 (Central)"
    )
  ) 
popCols <- c(
  rgb(0, 0, 0, 0.7), 
...
PCA clearly indicated some samples from the 2012 population were separating from the other 2012 samples which group with the 1996 population, and as such the collection points for the 2012 samples were investigated.
The tail on PC2 was clearly observed as representing points from a discrete region, and as such 2012 samples were broken into *Outer* or *Central* based latitude (vertical lines shown on the following plot) and longitude, reprresenting a box drawn around the 'Central' collection region

```{r pc2VsLat, fig.height=5, fig.width=6, fig.cap = "*Latitude against PC2 for samples collected in 2012. The tail observed on PC2 clearly associated with a specific collection region.*"}
pca$x %>% 
  as.data.frame() %>% 
  rownames_to_column("Sample") %>% 
  left_join(sampleID) %>% 
  dplyr::select(Sample, Population, Latitude, Longitude, splitPop, PC1, PC2) %>%
  dplyr::filter(Population == 2012) %>% 
  ggplot(aes(Latitude, PC2, colour = splitPop)) + 
  geom_point() + 
  geom_vline(
    xintercept = central$Latitude,
    linetype = 2,
    colour = "blue"
    ) +
  scale_color_manual(
...
```{r plotPCA, cache=TRUE, dependson='pca', fig.height=5, fig.width=6, fig.cap="*PCA showing population structure. Point size reflects the proportion of SNPs for which imputation was required, and the observed structure appeared independent of this.*"}
fs <- 14
ps <- 3
pca4Plot <- pca$x %>%
  as.data.frame() %>%
  ...
  left_join(sampleID) %>%
  dplyr::filter(!Sample %in% lowCall) %>%
  left_join(
    genotypes %>% 
...
pca4Plot %>%
    ggplot(aes(PC1, PC2, colour = splitPop, size = imputationRate)) +
...
zoomPlot <- ggmap(ggMap, extent = "normal", maprange = FALSE) +
  ...
  geom_point(
    ...
    aes(Longitude, Latitude, colour = splitPop),
    size = 0.9*ps) +
  geom_polygon(
...
get_map(zoomLoc, zoom = 15, maptype = "terrain", source = "google", color = "bw") %>%
  ...
  geom_rect(
    xmin = min(central$Longitude),
    xmax = max(central$Longitude),
    ymin = min(central$Latitude),
    ymax = max(central$Latitude),
...
oraRegions <- sampleID %>%
  dplyr::filter(Population == 2012) %>%
  mutate(Central = grepl("Central", splitPop)) %>%
...
A total of `r comma(sum(regionResults$p < 0.05))` SNPs were detected as showing a significant association between genotype and the collection region.
...
bind_rows(
  ...
  ggplot(aes(PC1, PC2, colour = splitPop)) +
  geom_point() +
  facet_wrap(~Status) +
  scale_colour_manual(values = popCols) +
  theme(
...
```{r Figure2_png, fig.cap="*Figure 2: Principal Components Analysis showing structures before removal of SNPs denoting collection region in the 2012 population, and after removal of these SNPs. Removal of these SNPs appears to have removed the internal structure within the 2012 population.*", out.width='80%'}
...
```{r vcfIDs, include = FALSE}
vcfIDs <- read.vcfR("../5_stacks/vcf/filtered.vcf.gz")@fix[,"ID"] %>%
  str_remove_all(":[\\+\\-]$") %>%
  str_split_fixed(pattern = ":", n = 2) %>%
  apply(MARGIN = 2, as.integer) %>%
  apply(MARGIN = 1, function(x){
    paste(x[[1]], x[[2]] - 1, sep = "_")
  })
```
...
grid.newpage()
list(
  Genotype = dplyr::filter(genotypeResults, FDR < alpha)$snpID,
  Allele = alleleResults %>%
    left_join(snps) %>%
    dplyr::filter(FDR < alpha) %>%
    .[["snpID"]],
  Bayescan = dplyr::filter(bayRes, qval < 0.05)$snpID
) %>% 
...
```{r metaP, fig.cap="*Comparison of p-values for SNPs detected in both analyses, with those considered as significant after meta-analysis shown in red.*"}
combinedResults %>%
  mutate(Sig = FDR < 0.05) %>%
...
```{r metaFreqPlot, fig.cap = "*Allele distributions in the 1996 and 2012 populations for SNPs identified under the meta-analysis.*"}
combinedResults %>%
  dplyr::filter(FDR < 0.05) %>%
  dplyr::select(snpID) %>%
  left_join(genotypes) %>%
