...
- After loading the data from the previous section, an additional fitering step was performed, where SNPs were required to have a non-zero MAF in the initial 1996 population.
...
```{r plotWithInset_png, echo = FALSE, fig.cap="*Figure 1: Collection points for all 2012 samples with colours showing sub-populations initially defined by PCA analysis and *k*-means clustering.*", out.width='80%'}
knitr::include_graphics(file.path("..", "figures", "Figure1.png"))
```

```{r zoomCentralPlot, fig.height=7, fig.width=7, fig.cap = "*Zoomed-in view of the central region for 2012 samples with colours showing sub-populations defined by PCA analysis. The region considered to be the Central Region is shaded in red. Due to overlapping GPS points a small amount of jitter has been added to the x-axis.*"}
...
As the samples collected in the outer regions appeared very similar to the 1996 population in the above plots, this would possibly indicate migration a very recent event as the genetic influence of this has not spread through the wider area.
...
In this way, any candidate SNPs obtained below will be less impacted by this structure, and will be more reflective of the intended variable under study, i.e. selection over time, as opposed to any internal structure of the 2012 population.
...
```{r comparePCA, echo=FALSE, results='hide'}
...
```{r Figure2_png, echo = FALSE, fig.cap="*Figure 2: Principal Components Analysis showing structures before removal of SNPs denoting collection region in the 2012 population, and after removal of these SNPs*", out.width='80%'}
...
genotypeResults %>%
  ...
  left_join(subset(snp2Gene, snpID %in% .$snpID) %>%
              as.data.frame() %>%
              dplyr::select(snpID, `Gene within 100kb` = Name)) %>% 
  mutate(position = comma(position)) %>%
  pander(justify = "llrrrrl",
         style = "rmarkdown",
         big.mark = ",",
         split.tables = Inf,
         caption = paste(
           "SNPs with adjusted p-values <", alpha, "when analysing by genotype."))
```
...
genotypeResults %>% 
    ...
    mutate(Genotype = c("AA", "AB", "BB")[Genotype + 1],
...
```{r manhattanAllele, echo = FALSE, fig.height=5, fig.width=8, }
...
```{r manhattanAllele_png, echo = FALSE, fig.cap = "Manhattan plot showing results for all SNPs on chromosomes 1:21 when analysing by allele frequencies. The horizontal line indicates the cutoff for an FDR of 5%, with SNPs considered significant under the Bonferroni adjustment shown in green.", out.width='80%'}
knitr::include_graphics(file.path("..", "figures", "FigureS3b.png"))
```


```{r plotMAF, echo=FALSE, fig.cap = "*Comparison of minor allele frequencies in both populations. SNPs with an FDR-adjusted p-value < 0.05 are coloured red, whilst those with a Bonferroni-adjusted p-value are additionally labelled.*"}
alleleResults %>%
...
```{r bayRes}
bayRes <- read_csv("../external/BayOut_FDR5.csv") %>%
  dplyr::rename(ID = X1) %>%
  mutate(variant.id = var2VCF[ID])
```

```{r plotVenn, echo=FALSE, fig.cap = "*Summary of results from all three methods, using an FDR of 0.05 for each approach*"}
grid.newpage()
list(
...
SNPs were mapped to GO terms and tested for enrichment.
In order to avoid double counting, GO terms were mapped to each SNP so that if multiple genes were within a given distance of a SNP, only one set of GO terms would be tested for that SNP.

```{r sigSnpGR}
sigSnpGR <- c(
  dplyr::filter(genotypeResults, FDR < alpha)$snpID,
  dplyr::filter(alleleResults, FDR < alpha)$snpID
) %>%
  unique() %>%
  enframe(value = "snpID") %>%
  dplyr::select(snpID) %>%
...
snp2GO <- allSnpsGR %>% 
  mcols() %>% 
  as.data.frame() %>% 
  dplyr::select(snpID, gene_id, Name) %>% 
  as_tibble() %>% 
  left_join(bm) %>%
  distinct(snpID, go_id, .keep_all = TRUE)
```
  ...
  lapply(function(x){
    tibble(go_id = x,
           ...
           Sig = sigSnpGo %>%
             ...
             distinct(snpID) %>%
             nrow(),
           Expected = nSig * N / nSnps) %>%
      mutate(p = c(nSig - Sig, Sig, nSnps - nSig - N, N) %>%
               matrix(nrow = 2) %>%
               fisher.test() %>%
               .[["p.value"]])
  }) %>%
...
sigSnpGo %>% 
  group_by(go_id) %>% 
  summarise(
    snps = list(unique(snpID)), 
    genes = list(unique(Name))
  ) %>%
  right_join(goRes) %>%
  dplyr::select(go_id, Ontology, Term, N, Sig, Expected, p, adjP, FDR, snps, genes) %>%
  as.data.frame() %>%
  ...
  pander(split.tables = Inf,
         caption = "*Enriched GO terms amongst the SNPs putatively under selection.*")
```

## Linkage
### Move to new markdown!!!

Linkage was calculated for each chromosome separately.
Testing is on Chr1

```{r}
gdsFile <- seqOpen(gdsPath)
```
...
ldList <- list()
for (i in 1:22){
  # Restrict to each chromosome
  seqSetFilterChrom(gdsFile, include = i)
  # Get all the variants then sample 20% of them
  allIDs <- tibble(
    variant.id = seqGetData(gdsFile, "variant.id"),
    chromosome = seqGetData(gdsFile, "chromosome"),
    position = seqGetData(gdsFile, "position")
  ) %>%
    sample_n(floor(nrow(.) / 8))  %>%
    arrange(chromosome, position)
  ...
  ld <- snpgdsLDMat(
    gdsobj = gdsFile, 
    sample.id = filter(sampleID, Population != 2010)$Sample,
    snp.id = allIDs$variant.id,
    slide = 0,
    num.thread = mc,
    with.id = TRUE)
  # Now add each data.frame to that list
  ldList[[i]] <- ld$LD %>%
    .[lower.tri(.)] %>%
    enframe(value = "LD") %>%
    dplyr::select(LD) %>%
    ...
    as_tibble() %>%
    left_join(allIDs %>% dplyr::select(ID1 = variant.id, Pos1 = position)) %>%
    left_join(allIDs %>% dplyr::select(ID2 = variant.id, Pos2 = position)) %>%
    mutate(bp = Pos2 - Pos1) %>%
...
```{r, fig.cap = "Example Linkage using Chr1. The smoothed line is plotted using a generalised additive model. Linkage appears limited to distances below 100kb"}
ldByDist %>%
  sample_n(1e6) %>%
...
```{r, fig.cap = "Zoomed in linkage for Chr1 and distances < 1Mb"}
ldByDist %>%
  filter(bp < 1e6) %>%
  ggplot(aes(bp / 1e3, LD^2)) +
  geom_point(alpha = 0.2) +
