...
## Initial Comparison of Reads with Alignments
...
```{r, echo=FALSE, fig.cap="*Comparison of library sizes before and after alignment. Given the filtering of alignments, this shows an acceptable rate of alignment across all samples*", fig.height=10, fig.width=12}
list(
  readTotals(deMuxFqc) %>%
...
```{r, echo=FALSE, fig.cap="*Alignment rate for each sample after filtering for uniquely mapped reads with high mapping qualities. The two samples gc2700 and gc2709 appear to be outliers.*", fig.height=9, fig.width=12}
list(
  readTotals(deMuxFqc) %>%
    mutate(Sample = str_remove(Filename, ".[12].fq.gz")) %>%
    group_by(Sample) %>%
...
```{r, echo=FALSE, fig.cap="*Difference in GC content when comparing each sample to Theoretical GC content from sampling the O. cuniculus genome.*", fig.height=10, fig.width=10}
plotGcContent(alnFqc, usePlotly = TRUE, dendrogram = TRUE, theoreticalGC = TRUE, GCobject = oryGC, species = "Ocuniculus")
```

```{r, echo=FALSE, fig.cap="*GC content of the complete set of samples. The three outlier samples are clearly evident.*"}
alnFqc %>%
...
```{r, echo=FALSE, fig.cap = "*Per base sequence content of all samples, with divergent patterns clearly identifying the outlier samples*", fig.height=15}
...
All identified low quality samples will be manually moved to a sub-folder indicating their low quality and will be excluded from the `stacks` pipeline.
...
list(
  ...
  mutate(
    `Alignment Rate` = percent(`Post-Alignment` / `Pre-Alignment`)
  ) %>%
  group_by(Population) %>%
  summarise(
  ...
  dplyr::select(Population, `Pre-Alignment`, contains("Align"), everything()) %>%
  pander(
    big.mark = ",",
    style = "rmarkdown",
    justify = "lrrrrr",
    ...
    split.table = Inf,
    caption = "Summary of alignment rates overall and by population"
  )
```
